...
$
  sum y_i = #calc.round(sumY, digits: 3) \
  sum x_(i 1) = #calc.round(sumX1, digits: 3)\
  sum x_(i 2) = #calc.round(sumX2, digits: 3)\
  sum x_(i 1)^2 = #calc.round(sumX1Squared, digits: 3)\
  sum x_(i 2)^2 = #calc.round(sumX2Squared, digits: 3)\
  sum x_(i 1) x_(i 2) = #calc.round(sumX1X2, digits: 3)\
  sum y_i x_(i 1) = #calc.round(sumYx1, digits: 3)\
  sum y_i x_(i 2) = #calc.round(sumYx2,digits: 3)
$
...
$
  overline(y) = (sum y_i)/n = #calc.round(sumY, digits: 3) / #n = #calc.round(meanY, digits: 3) \
  overline(x)_1 = (sum x_(i 1))/n = #calc.round(sumX1, digits: 3) / #n = #meanX1 \
  overline(x)_2 = (sum x_(i 2))/n = #sumX2 / #n = #calc.round(meanX2, digits: 3)
$
...
$
  S_y^2 = (sum y_i^2 - (sum y_i)^2/n)/(n-1) = (#calc.round(sumYSquared, digits: 3) - #calc.round(sumY, digits: 3)^2/#n)/(#n - 1) = #calc.round(varY, digits: 3) \
  S_y = sqrt(S_y^2) = sqrt(#calc.round(varY, digits: 3)"") = #calc.round(stdY, digits: 3) \
  S_(x_1)^2 = (sum x_(i 1)^2 - (sum x_(i 1))^2/n)/(n-1) = (#sumX1Squared - #sumX1^2/#n)/(#n - 1) = #calc.round(varX1, digits: 3) \
  S_(x_1) = sqrt(S_(x_1)^2) = sqrt(#calc.round(varX1, digits: 3)"") = #calc.round(stdX1, digits: 3) \
  S_(x_2)^2 = (sum x_(i 2)^2 - (sum x_(i 2))^2/n)/(n-1) = (#calc.round(sumX2Squared, digits: 3) - #sumX2^2/#n)/(#n - 1) = #calc.round(varX2, digits: 3) \
  S_(x_2) = sqrt(S_(x_2)^2) = sqrt(#calc.round(varX2, digits: 3)"") = #calc.round(stdX2, digits: 3)
$
...
$
  S_(x_1 y) = (sum x_(i 1) y_i - (sum x_(i 1) sum y_i)/n)/(n-1) = (#sumYx1 - #sumX1 * #calc.round(sumY, digits: 3) / #n)/(#n - 1) = #calc.round(covX1Y, digits: 3) \
  S_(x_2 y) = (sum x_(i 2) y_i - (sum x_(i 2) sum y_i)/n)/(n-1) = (#calc.round(sumYx2, digits: 3) - #sumX2 * #calc.round(sumY, digits: 3) / #n)/(#n - 1) \ = #calc.round(covX2Y, digits: 3) \
  S_(x_1 x_2) = (sum x_(i 1) x_(i 2) - (sum x_(i 1) sum x_(i 2))/n)/(n-1) = (#sumX1X2 - #sumX1 * #sumX2 / #n)/(#n - 1) = #calc.round(covX1X2, digits: 3)
$
...
$
  r_(x_1 y) = S_(x_1 y)/(S_(x_1) S_y) = #calc.round(covX1Y, digits: 3) / (#calc.round(stdX1, digits: 3) * #calc.round(stdY, digits: 3)) = #calc.round(rX1Y, digits: 3) \
  r_(x_2 y) = S_(x_2 y)/(S_(x_2) S_y) = #calc.round(covX2Y, digits: 3) / (#calc.round(stdX2, digits: 3) * #calc.round(stdY, digits: 3)) = #calc.round(rX2Y, digits: 3) \
  r_(x_1 x_2) = S_(x_1 x_2)/(S_(x_1) S_(x_2)) = #calc.round(covX1X2, digits: 3) / (#calc.round(stdX1, digits: 3) * #calc.round(stdX2, digits: 3)) = #calc.round(rX1X2, digits: 3)
$
...
$
  X' X = [#grid(
    ...
    [$#sumX2$], [$#sumX1X2$], [$#calc.round(sumX2Squared, digits: 3)$],
  )]
$
...
$
  "det"(X' X) = \ #n (#sumX1Squared #calc.round(sumX2Squared, digits: 3) - #sumX1X2^2) - \ #sumX1 (#sumX1 #calc.round(sumX2Squared, digits: 3) - #sumX2 #sumX1X2) + \ #sumX2 (#sumX1 #sumX1X2 - #sumX2 #sumX1Squared) = \ #detXX
$
...
$
  "adj"(X' X) = [#grid(
    inset: 4pt,
    columns: (auto, auto, auto),
    [$#adjXX11$], [$#calc.round(adjXX12, digits: 3)$], [$#adjXX13$],
    [$#calc.round(adjXX21, digits: 3)$], [$#calc.round(adjXX22, digits: 3)$], [$#adjXX23$],
...
$
  (X' X)^(-1) = [#grid(
    inset: 4pt,
    columns: (auto, auto, auto),
    [$#calc.round(invXX11, digits: 9)$], [$#calc.round(invXX12, digits: 9)$], [$#calc.round(invXX13, digits: 9)$],
    [$#calc.round(invXX21, digits: 9)$], [$#calc.round(invXX22, digits: 9)$], [$#calc.round(invXX23, digits: 9)$],
    [$#calc.round(invXX31, digits: 9)$], [$#calc.round(invXX32, digits: 9)$], [$#calc.round(invXX33, digits: 9)$],
  )]
$
...
$
  hat(beta) = [#grid(
    inset: 4pt,
    columns: (auto),
    [$#calc.round(beta0, digits: 9)$],
    [$#calc.round(beta1, digits: 9)$],
    [$#calc.round(beta2, digits: 9)$],
  )]
$
...
$
  hat(y)_i = #calc.round(beta0, digits: 9) + #calc.round(beta1, digits: 9) x_(i 1) + #calc.round(beta2, digits: 9) x_(i 2)
$

Se calculan los valores estimados $hat(y)_i$ para cada observación utilizando el modelo\
$hat(y)_i = #calc.round(beta0, digits: 9) + #calc.round(beta1, digits: 9) x_(i 1) + #calc.round(beta2, digits: 9) x_(i 2)$:
...
#grid(
  ...
  [$1$], [$2.4$], [$30$], [$3763.78$], [#calc.round(yHat1, digits: 6)], [#calc.round(error1, digits: 6)],
  [$2$], [$2.5$], [$20$], [$3763.78$], [#calc.round(yHat2, digits: 6)], [#calc.round(error2, digits: 6)],
  [$3$], [$2.2$], [$80$], [$940.95$], [#calc.round(yHat3, digits: 6)], [#calc.round(error3, digits: 6)],
  [$4$], [$2.0$], [$70$], [$940.95$], [#calc.round(yHat4, digits: 6)], [#calc.round(error4, digits: 6)],
  [$5$], [$2.0$], [$60$], [$940.95$], [#calc.round(yHat5, digits: 6)], [#calc.round(error5, digits: 6)],
  [$6$], [$2.0$], [$50$], [$940.95$], [#calc.round(yHat6, digits: 6)], [#calc.round(error6, digits: 6)],
  [$7$], [$2.1$], [$150$], [$6009.25$], [#calc.round(yHat7, digits: 6)], [#calc.round(error7, digits: 6)],
  [$8$], [$2.2$], [$140$], [$6009.25$], [#calc.round(yHat8, digits: 6)], [#calc.round(error8, digits: 6)],
  [$9$], [$2.0$], [$130$], [$6009.25$], [#calc.round(yHat9, digits: 6)], [#calc.round(error9, digits: 6)],
  [$10$], [$2.1$], [$120$], [$6009.25$], [#calc.round(yHat10, digits: 6)], [#calc.round(error10, digits: 6)],
  [$11$], [$2.0$], [$110$], [$6009.25$], [#calc.round(yHat11, digits: 6)], [#calc.round(error11, digits: 6)],
  [$12$], [$2.0$], [$100$], [$6009.25$], [#calc.round(yHat12, digits: 6)], [#calc.round(error12, digits: 6)],
  [$13$], [$2.1$], [$90$], [$6009.25$], [#calc.round(yHat13, digits: 6)], [#calc.round(error13, digits: 6)],
  [$14$], [$2.4$], [$80$], [$6009.25$], [#calc.round(yHat14, digits: 6)], [#calc.round(error14, digits: 6)],
  [$15$], [$2.1$], [$60$], [$6009.25$], [#calc.round(yHat15, digits: 6)], [#calc.round(error15, digits: 6)],
  [$16$], [$2.4$], [$30$], [$6009.25$], [#calc.round(yHat16, digits: 6)], [#calc.round(error16, digits: 6)],
  [$17$], [$2.6$], [$20$], [$6009.25$], [#calc.round(yHat17, digits: 6)], [#calc.round(error17, digits: 6)],
  [$18$], [$2.7$], [$10$], [$6009.25$], [#calc.round(yHat18, digits: 6)], [#calc.round(error18, digits: 6)],
)
...
$ sum_(i=1)^18 epsilon_i = #calc.round(sumErrors, digits: 9) approx 0 $
...
El modelo de regresión lineal múltiple desarrollado, $y_i = hat(beta)_0 + hat(beta)_1 x_(i 1) + hat(beta)_2 x_(i 2) + epsilon_i$, es un modelo *matemático estático de tipo analítico*. Según la clasificación por grado de abstracción, se considera un modelo simbólico, ya que representa las propiedades del sistema (resistencia eléctrica en función de la longitud y el inverso del diámetro al cuadrado) mediante una ecuación matemática. Por el método de solución, pertenece a los modelos matemáticos estáticos, ya que describe relaciones equilibradas entre las variables sin considerar cambios en el tiempo. Es analítico, pues se resuelve directamente mediante el método de mínimos cuadrados, obteniendo una solución exacta para los coeficientes $hat(beta)_0$, $hat(beta)_1$ y $hat(beta)_2$ a partir del sistema matricial $(X' X)^(-1) X' y$.
  ...
  $
    "SSE" = sum_(i=1)^18 epsilon_i^2 = #calc.round(sse, digits: 3)
  $
  ...
  $
    S_(y, 1, 2) = sqrt(("SSE")/(n - (k + 1))) = sqrt(#calc.round(sse, digits: 3) / (#n - (2 + 1))) = #calc.round(stdError, digits: 3)
  $

El error estándar de la estimación, $S_(y, 1, 2) = #calc.round(stdError, digits: 3)$, indica la dispersión promedio de los datos observados respecto a los valores predichos por el modelo $y_i = hat(beta)_0 + hat(beta)_1 x_(i 1) + hat(beta)_2 x_(i 2)$. Un valor de $S_(y, 1, 2) = #calc.round(stdError, digits: 3)$ sugiere una variabilidad moderada de los errores residuales, lo que implica que el modelo captura una parte significativa de la variación en la resistencia eléctrica ($y_i$), aunque existe cierto nivel de imprecisión debido al ruido inherente en las mediciones. Este valor es coherente con un modelo de regresión lineal múltiple que, tras la transformación de $x_2 = 1/("diámetro")^2$ y la eliminación de observaciones atípicas, logra un ajuste razonable ($R^2 approx 0.65$), pero indica que factores adicionales o ruido experimental podrían influir en las predicciones.
...
#grid(
  ...
  ..tableCalc.enumerate().map(((i, row)) => (
    [#(i + 1)], ..row.slice(0, 7).map(x => [#calc.round(x, digits: 6)])
  )).flatten()
)
...
$
  "SCR" = #calc.round(scr, digits: 3) \
  "STC" = #calc.round(stc, digits: 3) \
  R^2 = "SCR"/"STC" = #calc.round(scr, digits: 3) / #calc.round(stc, digits: 3) = #calc.round(rSquared, digits: 3)
$

El coeficiente de determinación múltiple, $R^2 = #calc.round(rSquared, digits: 3)$, indica que aproximadamente el $#calc.round(rSquared * 100, digits: 3) percent$ de la variación en la resistencia eléctrica ($y_i$) es explicada por las variables independientes longitud ($x_(i 1)$) y el inverso del diámetro al cuadrado ($x_(i 2)$) en el modelo $y_i = hat(beta)_0 + hat(beta)_1 x_(i 1) + hat(beta)_2 x_(i 2) + epsilon_i$. Este valor refleja un ajuste razonable, superando el umbral de 0.65, lo que sugiere que el modelo captura una proporción significativa de la variabilidad de los datos tras la transformación de $x_2$ y la eliminación de observaciones atípicas. Sin embargo, el $#calc.round(100 - rSquared * 100, digits: 3) percent$ restante de la variación no explicada podría atribuirse a factores no incluidos en el modelo, como variaciones en la resistividad del cobre o ruido experimental, lo que indica que, aunque el modelo es robusto, no explica completamente el comportamiento de la resistencia eléctrica.
...
Con $n = 18$, $k = 2$ y $R^2 = #calc.round(rSquared, digits: 3)$, se calcula:

#let rSquaredAdjusted = calc.round(1 - (1 - rSquared) * (n - 1) / (n - 2 - 1), digits: 30)

$
  R^2_"ajustado" = 1 - (1 - #calc.round(rSquared, digits: 3)) (#n - 1)/(#n - 2 - 1) = #calc.round(rSquaredAdjusted, digits: 3)
$

El coeficiente de determinación ajustado, $R^2_"ajustado" = #calc.round(rSquaredAdjusted, digits: 3)$, indica que aproximadamente el $#calc.round(rSquaredAdjusted * 100, digits: 3) percent$ de la variación en la resistencia eléctrica ($y_i$) es explicada por el modelo $y_i = hat(beta)_0 + hat(beta)_1 x_(i 1) + hat(beta)_2 x_(i 2) + epsilon_i$, ajustado por el número de variables predictoras ($k=2$) y el tamaño de la muestra ($n=18$). Comparado con el $R^2 = #calc.round(rSquared, digits: 3)$ ($#calc.round(rSquared * 100, digits: 3) percent$), el $R^2_"ajustado"$ es ligeramente menor debido a la penalización por el número de variables, reflejando un ajuste más conservador que considera la complejidad del modelo. Este valor confirma que el modelo es razonablemente robusto, aunque la diferencia de aproximadamente $#calc.round((rSquared - rSquaredAdjusted) * 100, digits: 3) percent$ entre $R^2$ y $R^2_"ajustado"$ sugiere una leve pérdida de eficiencia explicativa al incluir dos variables predictoras. Ambos coeficientes indican que el modelo, tras la transformación de $x_2$ y la eliminación de observaciones atípicas, captura una proporción significativa de la variabilidad, pero factores no modelados, como ruido experimental o variaciones en la resistividad, podrían explicar el porcentaje restante.

== Homocedasticidad e interpretación

== Normalidad e interpretación

Para validar el supuesto de normalidad en el modelo de regresión lineal múltiple $y_i = #calc.round(beta0, digits: 9) + #calc.round(beta1, digits: 9) x_(i 1) + #calc.round(beta2, digits: 9) x_(i 2) + epsilon_i$, se asume que los errores $epsilon_i$ siguen una distribución normal, $epsilon_i ~ (0, sigma^2)$. La suma de los errores residuales se calculó como:

$
  sum_(i=1)^18 epsilon_i = #calc.round(sumErrors, digits: 9)
$

El valor $sum_(i=1)^18 epsilon_i = #calc.round(sumErrors, digits: 9)$ es aproximadamente cero, lo que apoya el supuesto de que los errores tienen media cero. La varianza de los errores se estima mediante el error estándar al cuadrado, $S_(y, 1, 2)^2 = #calc.round(stdError * stdError, digits: 3)$, que representa $sigma^2$. Esto sugiere que los errores $epsilon_i ~ (0, #calc.round(stdError * stdError, digits: 3))$ cumplen razonablemente con el supuesto de normalidad, permitiendo que los intervalos de confianza y las pruebas de hipótesis basadas en el modelo sean válidos.

== Análisis del Modelo

Desde el ámbito de la ingeniería eléctrica, el modelo de regresión lineal múltiple 
$ y_i = #calc.round(beta0, digits: 9) + #calc.round(beta1, digits: 9) x_(i 1) + #calc.round(beta2, digits: 9) x_(i 2) + epsilon_i $ 
describe la resistencia eléctrica ($y_i$, en ohmios) de cables de cobre en función de su longitud ($x_(i 1)$, en cm) y el inverso del diámetro al cuadrado ($x_(i 2)$, en cm$""^(-2)$). El coeficiente $hat(beta)_0 = #calc.round(beta0, digits: 9)$ representa la resistencia base, que es baja y coherente con un intercepto físico cercano a cero. El coeficiente $hat(beta)_1 = #calc.round(beta1, digits: 9)$ indica que, por cada cm adicional de longitud, la resistencia aumenta en promedio #calc.round(beta1, digits: 9) ohmios, manteniendo $x_(i 2)$ constante, lo cual es consistente con la proporcionalidad directa de la resistencia a la longitud según la fórmula física $ R = rho (x_1)/(pi (x_2/2)^2) $. El coeficiente $hat(beta)_2 = #calc.round(beta2, digits: 9)$ refleja que un incremento en $x_(i 2) = 1/("diámetro")^2$ aumenta la resistencia en #calc.round(beta2, digits: 9) ohmios por cm$""^(-2)$, capturando el efecto inverso del área transversal. Con $R^2 = #calc.round(rSquared, digits: 3)$ ($#calc.round(rSquared * 100, digits: 3) percent$) y $R^2_"ajustado" = #calc.round(rSquaredAdjusted, digits: 3)$ ($#calc.round(rSquaredAdjusted * 100, digits: 3)) percent$, el modelo explica una proporción significativa de la variación en la resistencia, y el error estándar $S_(y, 1, 2) = #calc.round(stdError, digits: 3)$ indica una dispersión moderada de los errores. Este ajuste, logrado tras transformar $x_2$ y eliminar observaciones atípicas, permite predecir la resistencia con precisión aceptable para aplicaciones prácticas en diseño de circuitos eléctricos, aunque el $#calc.round(100 - rSquared * 100, digits: 3) percent$ de variación no explicada sugiere posibles influencias de factores como variaciones en la resistividad del cobre o imprecisiones en las mediciones.

= Conclusiones

El modelo de regresión lineal múltiple explica el $#calc.round(rSquared * 100, digits: 3) percent$ de la variación en la resistencia eléctrica de cables de cobre, con $R^2_"ajustado" = #calc.round(rSquaredAdjusted, digits: 3)$, siendo útil para diseño eléctrico aunque con limitaciones por ruido experimental.